...
class SimFunction(autograd.Function):
    ...
    def forward(
        ...
        x_init = np.load('/home/ubuntu/Github/DiffCloth/src/python_code/DataSort/npfiles/x_init_table_task.npy').flatten()

        out_x = [argX.reshape(-1, 3).copy()]
        out_v = [argV.reshape(-1, 3).copy()]
        fw_records = []

        "Disable this if you want to use the gradient optimizer for real2sim"
        # cppSim.sceneConfig.stepNum = -1
        ...
        for step in range(cppSim.sceneConfig.stepNum - 1):
            ...
            for i in range(int(len(argV) / 3) - 1):
                ...
                v[i * 3 + 1] = argV[i * 3 + 1] - 5 * a * math.sqrt(2 * h * 9.81 * 0.01)
