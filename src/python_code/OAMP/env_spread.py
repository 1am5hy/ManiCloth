...
from src.python_code.pySim.pySim_spread import pySim
...
class cloth_env:
    def __init__(self):
        ...
        self.scene.primitiveConfig = dfc.PrimitiveConfiguration.TABLE_HIGH
        ...
        self.scene.forwardConvergenceThresh = 1e-4
        self.scene.backwardConvergenceThresh = 5e-4
    ...
    def reset(self):
        ...
        #     np.save("/home/ubuntu/Github/DiffCloth/src/python_code/DataSort/npfiles/marker_spread_obs_24.npy", self.example)
    ...
    def get_obs(self):
        observe = self.sim.getStateInfo().x
        obs = observe[self.interest1]
        table_pos = np.load("/home/ubuntu/Github/DiffCloth/src/python_code/DataSort/npfiles/table_high_pos.npy")
        obs = table_pos - obs
        # obs = self.sim.getStateInfo().x
        return obs

    def get_rew(self, obs):
        traj = np.load("/home/ubuntu/Github/DiffCloth/src/python_code/DataSort/npfiles/obs_spread.npy", allow_pickle=True)

        # Include IOU and include the distance between AMP and current
        loss = 1
        rew = 1

        if self.reset_clock == 56:
    ...
    def step(self, action):
        ...
        if self.reset_clock == 57:
            self.scene.customAttachmentVertexIdx = [(0.0, [0])]
            self.sim = dfc.makeSimFromConf(self.scene)
            self.paramInfo = dfc.ParamInfo()
            self.paramInfo.x0 = self.last_x.flatten()
            self.sim.resetSystemWithParams(self.helper.taskInfo, self.paramInfo)
        ...
        # gp = np.load('/home/ubuntu/Github/DiffCloth/src/python_code/DataSort/npfiles/gp_spread_modified.npy')
        # #
        ...
        observe = self.x
        obs = self.get_obs()
        # self.example[self.reset_clock] = obs
        # obs = self.sim.getStateInfo().x
