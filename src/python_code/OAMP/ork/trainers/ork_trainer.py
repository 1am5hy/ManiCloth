...
# from agents.ork_agent import ORKAgent
from src.python_code.OAMP.ork.agents.ork_agent import ORKAgent


class ORKTrainer(RofuncAMPTrainer):
    def __init__(self, cfg, env, device, env_name, inference=False):
        BaseTrainer.__init__(self, cfg, env, device, env_name, inference)
        self.memory = RandomMemory(memory_size=self.rollouts, num_envs=self.env.num_envs, device=device)
        self.motion_dataset = RandomMemory(memory_size=10000, device=device)
        self.replay_buffer = RandomMemory(memory_size=10000, device=device)
        self.collect_observation = None
        self.agent = ORKAgent(cfg.train, self.env.observation_space, self.env.action_space, self.memory,
                              ...
                              target_observation_space=self.env.target_observation_space)
