...
parser.add_argument("--mode", type=str, default="Image")  # Available modes: Particle, Image, Graph
        ...
        with tqdm.trange(self.maximum_steps, ncols=80, colour='green') as self.t_bar:
            for i in self.t_bar:
                if i % 50 == 0 and custom_args.agent in ['PPOHorizon', 'SACHorizon']:
                    states = torch.unsqueeze(torch.as_tensor(self.env.reset_primitives().copy()), dim=0).to(self.device)
