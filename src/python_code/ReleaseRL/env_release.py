...
class cloth_env:
    ...
    def get_rew(self, obs, time):
        # pose = np.load("/home/ubuntu/Github/DiffCloth/src/python_code/ReleaseRL/np_files/hang_pose.npy").flatten()
        ...
        # Normalize loss to range 0 to 1
        if np.mean(y_value) < 5.5:
            # loss = 5.5 - y_value
            # weight = np.linalg.norm(5.5 - y_value)
            # loss = loss / weight
            # loss = np.mean(loss) * 10
            loss = 0
