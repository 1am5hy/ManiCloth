...
class Runner:
    ...
    def train(self):
        ...
        for epoch_i in tqdm(range(num_epochs)):
            ...
            if len(position_control) > 116:
                position_control = position_control[:116]
            ...
            markers = np.load('/home/ubuntu/Github/DiffCloth/src/python_code/real2sim/optitrack_data/npfiles/marker_path_folding.npy')
            if len(markers) > 116:
                markers = markers[:116]
            ...
            point1 = markers[:, 156] - x[:, 156]
            point2 = markers[:, 7] - x[:, 7]
            # print(markers.shape)
            loss1 = torch.nn.functional.mse_loss(markers[:, 156], x[:, 156], reduction='sum').clone().detach()
            loss2 = torch.nn.functional.mse_loss(markers[:, 7], x[:, 7], reduction='sum').clone().detach()
            loss = torch.nn.functional.mse_loss(markers, x, reduction='sum') + 0.25 * loss1 + 0.25 * loss2
            print(loss)
