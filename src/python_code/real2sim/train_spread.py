...
from sim_for_spread import SimDeform
...
class Runner:
    ...
    def train(self):
        ...
        for epoch_i in tqdm(range(num_epochs)):
            ...
            if len(position_control) > 75:
                position_control = position_control[:75]

            x_init = np.load('/home/ubuntu/Github/DiffCloth/src/python_code/DataSort/npfiles/x_init_spread.npy')
            ...
            markers = np.load('/home/ubuntu/Github/DiffCloth/src/python_code/DataSort/npfiles/marker_spread_3.npy')
            ...
            loss = torch.nn.functional.mse_loss(markers, x, reduction='sum')
    ...
    def update_learning_rate(self, scale_factor=1):
        learning_factor = .75 ** (self.iter_step // 400)
    ...
    def upsample(self, marker, particle):
        ...
        # particle[:, 80] = marker[:, 2]
        # particle[:, 82] = marker[:, 3]
        # particle[:, 110] = marker[:, 4]
        # particle[:, 112] = marker[:, 5]
        particle[:, 210] = marker[:, 6]
        particle[:, 224] = marker[:, 7]
        # print(marker[:, 7])
        # print(particle[:, 870])
