...
def lossFunction(xvPairs):
    ...
    for (x_last, v_i) in xvPairs:
        for (i, (idx1, idx2)) in enumerate(CLIP_DIR_VERTEX_PAIR):
            ...
            cosine = torch.clamp(torch.dot(torch.nn.functional.normalize(dir, dim=0),
                                           torch.nn.functional.normalize(dirGoaltorch, dim=0)), max=0.5, min=None)
    ...
    loss = {'succeed': succeed, 'target': targetLoss, 'stretch': stretchPenalty, 'direction': directionPenalty,
            'total': stretchPenalty + targetLoss + directionPenalty}
    return loss
...
def simulateAndGetLoss(x0a0pairs, render=True):
    ...
    for (i, (x0_shift_torch, a0_shift_torch, _)) in enumerate(x0a0pairs):
        sim.resetSystem()
        xvPairs = common.forwardSimulation(sim, x0_shift_torch.clone(), v0_torch.clone(), a0_shift_torch.clone(),
                                           getState, controller, pySim)
...
def getValidationLosses(epoch, expName, render=False, saveImage=True, saveSimulation=True):
    ...
    for (i, (x0a0Deg, loss, xvPairs)) in enumerate(zip(X0A0pairs_eval, losses, simulations)):
        ...
        if saveImage:
            ...
            identifier = 'epoch-{:d}-deg{:.1f}-{:.1f}-loss-{:.2f}'.format(epoch, degrees[0], round(degrees[1]),
                                                                          loss['total'].item())
...
def loadCheckpoint(path, epoch):
    ...
    epochStart, testMinLoss, trainMinLoss, trainLosses, testLosses = checkpoint['epoch'], checkpoint['testMinLoss'], \
    checkpoint['trainMinLoss'], checkpoint['trainLosses'], checkpoint['testLosses']
    successLog, trainBestEpoch, testBestEpoch = checkpoint['successLog'], checkpoint['trainBestEpoch'], checkpoint[
        'testBestEpoch']
...
if not (eval_mode):
    ...
    if not (train_resume):
        ...
        epochStart, testMinLoss, trainMinLoss, trainLosses, testLosses, successLog, trainBestEpoch, testBestEpoch = loadCheckpoint(
            exp_path, '{}.pth'.format(loadEpoch))
    ...
    for epoch in range(epochStart, args.epochNum):
        ...
        logFile.close()
        logFile = open(exp_path / "log.txt", "a")
        loss = trainStep(sim, optimizer, trainSeqSampleNum=1, render=args.render and (epoch > 0) and (epoch % 10 == 0))
        trainLosses.append(loss)
        if (loss < trainMinLoss):
            trainMinLoss, trainBestEpoch = loss, epoch
            saveEpoch(isBestTrain=True, isBestVal=False)
        utils.log("Train: loss: {} minLoss: {} bestEpoch: {} norm:{} \n".format(loss, trainMinLoss, trainBestEpoch,
                                                                                nn.utils.clip_grad_norm_(
                                                                                    controller.parameters(), 1.0)),
                  not (eval_mode), logFile)
        evalLoss, evalSuccessNum = getValidationLosses(epoch, expName, render=(epoch % 10 == 0) and (args.render),
                                                       saveImage=True, saveSimulation=False)
        ...
        utils.log(
            "Test: loss: {} minLoss: {} bestEpoch:{}, evalSuccessNum:{}/{} rate:{}\n".format(evalLoss, testMinLoss,
                                                                                             testBestEpoch,
                                                                                             evalSuccessNum,
                                                                                             len(X0A0pairs_eval),
                                                                                             evalSuccessNum * 1.0 / len(
                                                                                                 X0A0pairs_eval)),
            not (eval_mode), logFile)
    ...
    epochStart, testMinLoss, trainMinLoss, trainLosses, testLosses, successLog, trainBestEpoch, testBestEpoch = loadCheckpoint(
        exp_path, '{}.pth'.format(loadEpoch))
    evalLoss, evalSuccessNum = getValidationLosses(epochStart, expName, render=args.render, saveImage=True,
                                                   saveSimulation=True)
