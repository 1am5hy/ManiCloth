...
#pragma omp parallel for if (OPENMP_ENABLED)
        for (int i = 0; i < Constraint::CONSTRAINT_NUM; i++) {
//          std::cout << "(" << sysMat[currentSysmatId].A_t_pertype[i].rows() << ", " << sysMat[currentSysmatId].A_t_pertype[i].cols() << ")";;
//          std::cout << "(" << projections_pertype[i].rows() << ", " << projections_pertype[i].cols() << ")";;
//          std::cout << "(" << std::sqrt(*(k_stiff_arr[i])) << ", " << std::sqrt(*(k_stiff_arr[i])) << ")";;
...
//        std::cout << "(" << A_t_A_weightless_times_xnew.rows() << ", " << A_t_A_weightless_times_xnew.cols() << ")";;
//        std::cout << "(" << dA_t_times_p_dk.rows() << ", " << dA_t_times_p_dk.cols() << ")";;
...
//      primitives.push_back(&plane1);
    ...
    case PLANE_AND_SPHERE: {
      primitives.push_back(&leftLowerArm);
      break;
    }


    case BIG_SPHERE: {
      primitives.push_back(&rightLowerArm);
      break;
    }
...
//      rightUpperArm.center = Vec3d(0, 0, 0);
      plane1.mu = 0.5;
//      rightUpperArm.mu = 0;
//      bowl.mu = 0;
    break;
...
//      sphere2.rotates = false;
        leftLowerArm.center = Vec3d(0, 0, 0);
        leftLowerArm.mu = 0.8;
      break;
    }
